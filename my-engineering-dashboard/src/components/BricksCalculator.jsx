...
const brickPresets = {
  ...
  india: { name: "Standard India", l: 19, w: 9, h: 9, unit: 'cm' },
  us_modular: { name: "US Modular", l: 7.625, w: 3.625, h: 2.25, unit: 'in' },
  uk_standard: { name: "UK Standard", l: 215, w: 102.5, h: 65, unit: 'mm' },
  australia: { name: "Standard Australia", l: 230, w: 110, h: 76, unit: 'mm' }
};

export default function BricksCalculator() {
  const [inputs, setInputs] = useState({
    wallL: 15, wallH: 10, wallT: 9, wallUnit: 'ft',
    openingsArea: 20, 
    brickL: 19, brickW: 9, brickH: 9, brickUnit: 'cm',
    ratio: 4,
    brickWastage: 5,     
    brickPrice: 8,       
    cementPrice: 400,    
  ...
  const edgesMeshRef = useRef(null); 
  const gridRef = useRef(null);
  const animationDataRef = useRef([]);

  const handleInputChange = (e) => {
    const { name, value } = e.target;
    // FIX: Enforce numbers to be > 0 to prevent crashes
    let parsedValue = ['wallUnit', 'brickUnit', 'currency'].includes(name) ? value : parseFloat(value) || 0;
    if (typeof parsedValue === 'number') parsedValue = Math.max(0, parsedValue);
    
    setInputs(prev => ({ ...prev, [name]: parsedValue }));
    if (['brickL', 'brickW', 'brickH', 'brickUnit'].includes(name)) setPreset('custom');
  };
  ...
  const applyPreset = (e) => {
    ...
    setPreset(p);
    if (p !== 'custom') {
      const data = brickPresets[p];
      setInputs(prev => ({ ...prev, brickL: data.l, brickW: data.w, brickH: data.h, brickUnit: data.unit }));
    }
  };
  ...
  const calculateAndRender = () => {
    const { wallL, wallH, wallT, wallUnit, openingsArea, brickL, brickW, brickH, brickUnit, ratio, brickWastage, brickPrice, cementPrice, sandPrice, laborRate } = inputs;
    if(!wallL || !wallH || !wallT || !brickL || !brickW || !brickH) return;

    const wMult = unitToMeter[wallUnit];
    const bMult = unitToMeter[brickUnit];
    const sqMult = sqUnitToSqMeter[wallUnit];

    const wL_m = wallL * wMult;
    const wH_m = wallH * wMult;
    const wT_m = wallT * wMult;
    let openings_m2 = openingsArea * sqMult;
    ...
    const bL_m = brickL * bMult;
    const bW_m = brickW * bMult;
    const bH_m = brickH * bMult;
    const mT_m = 0.01; 
    ...
    if (instancedMeshRef.current) { scene.remove(instancedMeshRef.current); instancedMeshRef.current.dispose(); }

    const rows = Math.floor(wH_m / (bH_m + mT_m));
    const cols = Math.floor(wL_m / (bL_m + mT_m));
    const depthLayers = Math.max(1, Math.round(wT_m / bW_m));
    ...
    if (theoreticalBricks > 30000) {
        setVisualWarning(`Massive Scale! 3D Visualizer capped to prevent lagging. Engineering math remains exact.`);
    ...
    const cutoutDim = Math.sqrt((openings_m2 / totalWallArea_m2) * (actualWidth * actualHeight));
    const cutW = cutoutDim;
    const cutH = cutoutDim;
    ...
    const brickColors = ['#d94833', '#c9402a', '#e0543e', '#b53722', '#e86654'];
    ...
    for (let z = 0; z < renderDepth; z++) {
      for (let y = 0; y < renderRows; y++) {
        ...
        for (let x = 0; x < rowCols; x++) {
          ...
          const isInsideCutout = Math.abs(xPos) < (cutW/2) && Math.abs(targetY - (actualHeight/2)) < (cutH/2);
          ...
          if (!isInsideCutout) {
            ...
            if (!isBlueprint) {
                color.set(brickColors[Math.floor(Math.random() * brickColors.length)]);
    ...
    setResults({
      wL_m, wH_m, wT_m, openings_m2, bL_m, bW_m, bH_m, mT_m, sumRatio,
  ...
  return (
    <div className="flex flex-col xl:flex-row gap-6 p-4 md:p-6 bg-[#020617] text-slate-100 min-h-screen relative overflow-hidden font-sans print:p-0 print:bg-white print:text-black print:overflow-visible print:min-h-0 print:block">
        ...
        <div className="overflow-y-auto custom-scroll pr-1 space-y-4 flex-1 pb-4">
            ...
            <div className="bg-slate-800/40 p-4 rounded-xl border border-slate-700/50 hover:border-cyan-500/30 transition-colors group">
                <div className="flex items-center justify-between mb-3">
                    ...
                    <select name="wallUnit" value={inputs.wallUnit} onChange={handleInputChange} className="bg-slate-900/80 border border-slate-600/50 text-[10px] rounded-md px-1.5 py-1 outline-none text-white cursor-pointer focus:ring-1 focus:ring-cyan-500 transition-all">
                ...
                <div className="pt-3 border-t border-slate-700/50">
                    <label className="flex justify-between items-center text-[10px] text-slate-300 uppercase mb-1">
                        ...
                        <span className="text-cyan-500 font-mono text-[9px]">sq {inputs.wallUnit}</span>
                    </label>
                    <input type="number" step="1" name="openingsArea" value={inputs.openingsArea} onChange={handleInputChange} className="w-full bg-slate-900/80 border border-slate-700/80 rounded-lg p-2 text-xs focus:ring-1 focus:ring-cyan-500 outline-none text-white transition-all hover:bg-slate-800" placeholder="Area to subtract..." />
                </div>
            </div>
            ...
            <div className="bg-slate-800/40 p-4 rounded-xl border border-slate-700/50 hover:border-orange-500/30 transition-colors group">
                <div className="flex items-center justify-between mb-3">
                    ...
                    <select value={preset} onChange={applyPreset} className="bg-slate-900/80 border border-slate-600/50 text-[10px] rounded-md px-1.5 py-1 outline-none text-white cursor-pointer focus:ring-1 focus:ring-orange-500 transition-all max-w-[100px] truncate">
                        {Object.entries(brickPresets).map(([key, data]) => (<option key={key} value={key}>{data.name}</option>))}
                    </select>
                </div>
                <div className="grid grid-cols-3 gap-2 mb-3">
                  <div>
                    <label className="block text-[9px] text-slate-400 mb-1 uppercase">L ({inputs.brickUnit})</label>
                    <input type="number" name="brickL" value={inputs.brickL} onChange={handleInputChange} className="w-full bg-slate-900/80 border border-slate-700/80 rounded-lg p-2 text-xs focus:ring-1 focus:ring-orange-500 outline-none text-white" />
                  </div>
                  <div>
                    <label className="block text-[9px] text-slate-400 mb-1 uppercase">W ({inputs.brickUnit})</label>
                    <input type="number" name="brickW" value={inputs.brickW} onChange={handleInputChange} className="w-full bg-slate-900/80 border border-slate-700/80 rounded-lg p-2 text-xs focus:ring-1 focus:ring-orange-500 outline-none text-white" />
                  </div>
                  <div>
                    <label className="block text-[9px] text-slate-400 mb-1 uppercase">H ({inputs.brickUnit})</label>
                    <input type="number" name="brickH" value={inputs.brickH} onChange={handleInputChange} className="w-full bg-slate-900/80 border border-slate-700/80 rounded-lg p-2 text-xs focus:ring-1 focus:ring-orange-500 outline-none text-white" />
                  </div>
                </div>
      ...
      <div className="w-full xl:w-[calc(100%-404px)] bg-slate-900/40 backdrop-blur-xl rounded-3xl shadow-2xl border border-slate-700/50 flex flex-col overflow-hidden relative z-10 xl:h-[88vh] print:w-full print:h-auto print:border-none print:shadow-none print:bg-transparent print:overflow-visible print:block">
        ...
        {/* FIX: ADDED print layout classes here to ensure it prints perfectly */}
        {results && mode === 'math' && (
              ...
              <div className="space-y-8 max-w-4xl mx-auto">
                 ...
                 <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 print:shadow-none print:border-black print:border-2">
                    ...
                    <div className="font-mono text-sm space-y-3 text-slate-700 bg-slate-50 p-4 rounded-lg print:bg-transparent print:p-0">
                        ...
                        <p><span className="text-slate-400 print:text-black font-bold">Total Wall Area</span> = {results.wL_m.toFixed(4)} × {results.wH_m.toFixed(4)} = <span className="font-bold text-slate-900">{(results.wL_m * results.wH_m).toFixed(4)} m²</span></p>
                        <br/>
                        <p><span className="text-slate-400 print:text-black font-bold">Net Wall Area</span> = Total Wall Area - Openings Area</p>
                        <p><span className="text-slate-400 print:text-black font-bold">Net Wall Area</span> = {(results.wL_m * results.wH_m).toFixed(4)} - {results.openings_m2.toFixed(4)} = <span className="font-bold text-slate-900">{results.netWallArea_m2.toFixed(4)} m²</span></p>
                        <br/>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8 print:block">
                    <div className="print:mb-6">
                        ...
                        <table className="w-full text-sm">
                            <tbody>
                                <tr className="border-b border-slate-100 print:border-slate-300"><td className="py-2 text-slate-500 print:text-black">Wall Dimensions</td><td className="py-2 font-medium text-right">{inputs.wallL}×{inputs.wallH}×{inputs.wallT} {inputs.wallUnit}</td></tr>
                                <tr className="border-b border-slate-100 print:border-slate-300"><td className="py-2 text-slate-500 print:text-black">Openings Subtracted</td><td className="py-2 font-medium text-right text-rose-500 print:text-black">- {inputs.openingsArea} sq {inputs.wallUnit}</td></tr>
                                ...
                                <tr><td className="py-2 text-slate-500 print:text-black">Brick Preset</td><td className="py-2 font-medium text-right capitalize">{preset.replace('_', ' ')}</td></tr>
                            </tbody>
                        </table>
                    </div>
                    <div>
                ...
                <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden mb-8 print:border-black print:shadow-none">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="text-sm divide-y divide-slate-100 print:divide-slate-300">
                            <tr>
                                <td className="p-4"><p className="font-bold text-slate-800">Clay Bricks</p><p className="text-xs text-slate-500 mt-0.5 print:text-black">Includes {inputs.brickWastage}% wastage buffer</p></td>
